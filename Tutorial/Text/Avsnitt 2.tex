...
rysligt praktiskt! \\
%TODO: normalt sett borde \\ ... \newline bytas ut more bara en vanlig tomrad (här och nedan)
%------------------------------------------------------------------
...
\textbf{Övning 1:} Här följer ett kodskelett till en funktion som undersöker om ett system är Linjärt. Fyll i det som saknas.
...
%**TODO: Detta är inte parse-korrekt ("->" kan inte börja i första kolumnen)
\begin{minted}{haskell}
...
Ett tips när du beskriver signaler är att skriva multiplikation
...
använda $*$ för multiplikation där, medans faltning ofta betecknas med tex \emph{conv}, från engelskans convolution, eller liknande.


Faltning är en operator som det kan vara lite besvärligt att få en intuitiv känsla för. I det här avsnittet kommer vi endast gå in lite löst på \textsl{varför} den fungerar som den gör och mest konstatera \textsl{att} den fungerar. Men vi tar och benar ut den i stora drag i alla fall.
...
%**TODO: "ds" har hamnat på fel ställe - borde vara före sista likhetstecknet.
...
%TODO: notationen med [n-k] behöver förklaras mer (eller ändras).
...
och impulssvaret $h(t)$, kan systemets utsignal $y(t)$  beräknas från
$$y(t) = x(t) * h(t) $$
...
discConvolution s0 s1 start stop = ...
\end{minted}
