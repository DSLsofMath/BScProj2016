...
används på ett otydligt sätt i undervisningssammanhang och att man
antar att studenterna är betydligt mer bevandrade i
...
matematiska koncept för en dator, om en dator förstår konceptet är förklaringen
...
tillräckligt bekväma i det matematiska språket, se \nameref{bil:exam_intervju}.
...
påbörjades år 2014 ett pedagogiskt projekt på Chalmers, \textit{\gls{dslm}},
...
studenternas kunskaper inom programmering samt även underlätta för dem
...
till att ta fram läromaterial till endast en kurs, \gls{tss}. Läromaterialet
som tagits fram är ett
kompletterande material och är inte tänkt att kunna ersätta kursen
eller dess material.  Något
material till kursen \gls{regler} har inte tagits fram, annat än
indirekt i form av förkunskaper.
...
skriver över varandras arbete. Så länge inte två versioner av materialet innehåller
...
För ytterligare information om \gls{Git} se hemsidan
\url{https://git-scm.com}.

\gls{GitHub} är en hemsida som använder \gls{Git}. \gls{GitHub} gör det möjligt
...
\gls{GitHub} förenklar kommunikationen mellan användare och utvecklare
genom att tillhandahålla verktyg som låter användare kommentera på
enskilda versioner av källkoden, följa upp ärenden och problem samt
dokumentera sitt projekt. \gls{GitHub} har även verktyg för att
...
kan godkänna. Eftersom materialet lagras offentligt på \gls{GitHub} så är
den lättåtkomlig för andra utvecklare vilket underlättar vid framtida
vidareutveckling.

För ytterligare information om \gls{GitHub} se hemsidan
\url{https://github.com}.

\subsection{Funktionell programmering och Haskell}
...
bryta ned ett problem beror på hur enkelt man sedan kan sammanfoga
...
för mycket virtuellt minne och terminerar.
...
att implementera ett \gls{edsl} har sina för- och nackdelar då det
...
ett läromaterial som kan fungera som ett komplement till kursen \gls{tss}
...
i detalj i boken \textit{Motivational Design for Learning and Performance}
...
förändring i lärarens röstläge, temperatur i en undervisningssal eller ett skämt.
...
kunskapsbas om matematik som man själv. Detta gör, enligt Sussman och
...
krävs för att otvetydigt förklara ett matematiskt koncept, nämligen att försöka
...
det att materialet skulle samlas på en hemsida. På detta sätt kunde
man enkelt samla text, bilder och programmeringskod och presentera dem
på ett överskådligt sätt för studenterna i målgruppen.
...
text. Texten ska vara skriven på ett sådant sätt att den är lätt att ta till sig
...
förståelse av komplexa tal är en viktig förkunskap till \gls{tss}. Detta var
en anledning till att
gruppen valde att inleda med repetition av detta ämne. En annan
...
Det fjärde och sista avsnittet tillägnades Fourierserier och
...
Det finns många didaktiska modeller som kan användas vid utveckling
...
sina vanliga studier. Därför ansåg gruppen att ARCS-modellen var
...
först varför läromaterialet som helhet är relevant för studentens
...
övningarna så inledes läromaterialet i det första avsnittet med enklare
...
avslutas till sist med en lista på vad studenten har lärt
sig i avsnittet. Studenten får alltså både ett erkännande av sin kunskap och
...
för ett avsnitt först utvecklats fristående från det
...
så att hen kan köra sin egen kod direkt och testa sina lösningar utan att
behöva implementera bakomliggande kod som är irrelevant för inlärningen.
...
delen. Det komplexa talet \(z = 1 + 2j \) representeras i vår
\gls{dsl} som:
\begin{minted}{haskell}
  Complex 1 2
\end{minted}
De implementerade komplexa talen ska bete sig som matematikens
...
att skriva samma kod om och om igen, samtidigt som det gör koden mer
...
\url{https://github.com/DSLsofMath/BScProj/tree/master/Tutorial/Kod}
...
implementeras på detta sätt ger svar som är mer matematiskt korrekta
...
Efter att utkast till läromaterialets avsnitt utvecklats gjordes dessa om till
LaTeX-format. Anledningen till detta var att det var ett
förhållandevis enkelt sätt att få en överskådlig
struktur och ett bra utseende på läromaterialet.

När avsnitten i läromaterialet var färdiga så skrevs de om
till HTML, med hjälp av Pandoc, för att kunna skapa en hemsida för att
materialet  lätt\-åtkomligt. Pandoc är ett program som översätter
...
hemsidan. Om projektgruppen hade haft mer efarenhet av att arbeta med HTML skulle
man kunnat redigera i LaTeX för att sedan generera HTML koden igen,
men det hittades ingen simpel lösning till att göra detta och samtidigt
behålla strukturen på läromaterialet.

För uppgifterna i läromaterialet användes jQuery, vilket är ett JavaScript-
bibliotek, för att göra det lättare att arbeta med JavaScript. Anledningen
till detta var att projektgruppen ansåg att JQuery
var mer lättläsligt än vanlig JavaScript och det var därmed
lättare att felsöka i koden.

Beträffande designen av hemsidan användes verktyget
Bootstrap. Detta för att Bootstrap har CSS kod, som används vid design
av hemsidor, som man kan använda sig av. Bootstrap gör det även lätt
att skapa en responsiv hemsida, vilket gör att hemsidan skalas om
efter skärmstorleken utan att spendera mycket tid för att få en läsbar
design. På grund av den responsiva hemsidan blir det möjligt att
använda den skärm man har tillgänglig, vare sig det är en dator
eller mobil, utan att hemsidan blir dåligt optimerad. På detta vis undviks
att läsaren distraheras bort från texten på grund av dålig optmering.
...
avsnitt \internref{sec:efterforskning}. De studenter som sedan var villiga att
...
fungerade så att testpersonerna fick ta del av materialet, arbeta med
...
inte blev färdigt i tillräckligt god tid för att ge testgruppen en rimlig
...
var planerade och påbörjade men detta fick bortprioriteras på grund av tidsbrist.
...
beskriver \textit{Udda och Jämna signaler}. De övriga avsnitten har samma 
grundläggande upplägg som detta. Delavsnittet inleds med
...
läromaterialet, vilket förorsakde att projektgruppen inte hade
...
studenter ta fram den här typen av läromaterial. En nackdel med detta är att
...
som vår projektgrupp har behövt göra. En följd av detta blir dock att
...
definieras exakt för ett antal givnatypfall av insignaler som definieras var för
sig. Detta ger mer korrekta svar, men inte samma möjlighet att
sammanfatta
...
implementera Fouriertransform. Det finns emellertid utkast till
...
bidrag till att minska gapet mellan matematik och programmering och 
bidra till dessa konsekvenser.
...
läromaterialet in hela kursen \gls{tss} i stora drag.
...
resultat. Det skulle också vara värdefullt att testa produkten
tillsamans med kursen \gls{tss}.
...
kursen \gls{tss} inspirerat av \gls{dslm} som uppfyller
...
för rapporten och är även tömd på all information som kan länkas till
enskilda individer.
