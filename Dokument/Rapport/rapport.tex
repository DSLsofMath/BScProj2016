...
till studenter på det datatekniska programmet på Chalmers och har som syfte att
...
matematisk korrekthet, matematiska bevis, avancerade matematiska
...
ämnet. Detta beslut baserades även på \textit{Didaktik för
...
på olika sätt. En ytligt inlärd kunskap eller ett beteende som inte
...
Reglerteknik har en ovanligt hög andel studenter som inte blir
godkända på tentamen. Under perioden 2010 till 2016 blev i genomsnitt
...
%TODO: mer direkt: "beskriver vi kortfattat"
...
specifik domän. Att skapa domänspecifika språk är ingenting nytt och
det finns flera exempel på DSL som HTML, MATLAB och Game Maker
Language.

Det som är grundtanken med DSL är att det är relativt enkelt att
...
fristående språk som analyseras till någon typ av maskinkod eller att
...
typer i värdspråket. Båda sätten att beskriva DSL har sina fördelar
...
typer eftersom man bara måste kunna representera dem med värdspråket
...
%TODO: Skriv om "fixeras dessutom de givna parametrarna till en funktion ..." - jag förstår inte. Det är nog enklare att ge ett exempel - som plusEtt = (1+) eller dubbel = (2*).
...
\textit{Fouriertransformen} är en transform i diskret eller
kontinuerlig tid som transformerar en funktion från tidsdomänen till
frekvensdomänen. \textit{Fouriertransformen} är definierad enligt
följande:
...
Som nämndes i inledningen har projektet “Matematikens domänspecifika
språk” som syfte att utveckla läromaterial som kan fungera som ett
...
kommer i kontakt med det. Det är alltså viktigt att ett läromedel är
...
%TODO: "Slutgiltligen" := "Slutligen" (gissar jag)
...
\cite{sussman2002role}. De resonerar att detta troligen kan bero på
...
En bra lösning, som föreslås i artikeln, på hur man hittar den
...
Charles Wells publicerade under 1995 en artikel
...
material till en kurs. Dessa 10 steg är som följer:
...
\textit{Utveckling - Inledande efterforskning och förundersökning},
arbetade fram en taktik enligt ARCS-modellen, se \textit{Utveckling
- Didaktik}, utvecklade och testade materialet, se avsnitt {Utveckling
- Testning}, utvärderat och kompletterat det.

Gruppen hade även två inspirationskällor i form av
\url{http://betterexplained.com/} och \cite{learnyouahaskell}.
...
Se Bilaga \ref{bil:exam_intervju} för en sammanfattning av denna
...
Länken till enkäten på \textit{webbenkater.com} skickades ut till alla
datastudenter från årgångarna 2012-2014 och gruppen erhöll totalt 77
svar. Se \textit{bilaga 1} för ett utdrag ur dessa svar.
...
inlärning följer i nästa avsnitt, \textit{Didaktik i läromaterialet}.
...
utantill. \textit{Didaktik för ingenjörslärare} beskriver aktivt
...
och uppgifter. I ett kapitel får studenterna fylla i ett relativt
stort antal kryssfrågor och i ett annat lägger vi stort fokus på en
...
TSS som behandlas i läromaterialet. Se avsnittet \textit{Inledande
 forskning} för mer information om hur dessa delar valts ut.
...
Här skapades även hjälpfunktioner, vilka är de funktioner man finner
...
%TODO: typfel - saknas någon parentes. Jmf. Tutorial/Kod/ComplexNumbers.hs:127
\begin{minted}{haskell}
...
dator kan dock i praktiken inte hantera kontinuerliga funktioner utan
endast diskreta.
...
skickade ut enkäten som beskrivs i avsnitt \textit{Utveckling av
 läromaterialet - Inledande efterforskningar och förstudier}. De som
sedan ville deltaga i testandet av vår produkt lämnade en
mailadress. Detta resulterade i en testgrupp på 13 personer.

%TODO: Jag skulle föredra "skicka" framför "maila" (som ordlistan stavar "mejla")
...
\textit{Produktspecifikation}, att ta fram ett kompletterande
...
granskats av den testgrupp som beskrivs i avsnittet \textit{Utveckling
 av läromaterialet - Test av läromaterialet}. Det sista avsnittet har
inte granskats av testgruppen, utan endast av projektgruppen själva
samt projektgruppens handledare. Orsaken till detta beskrivs i
...
Under intervjun med examinatorerna, se \textit{bilaga 2}, framgick det
...
begriplig. \textit{The Fastest Fourier Transform in the
 West} %https://hackage.haskell.org/package/fft-0.1.8.3/docs/Math-FFT.html
...
% \textit{Utveckling av läromaterialet - Implementationen av vårt DSL}
...
%TODO: "pattern matching" = mönstermatchning (ni kanske vill ange båda första gången ni använder ordet)
...
samma möjlighet att
%TODO: ?? jag förstår inte "sammanfatta och abstrahera området" ??
sammanfatta och abstrahera området som en diskret approximation. Denna
...
större möjlighet att abstrahera och sammanfatta kärnan i ämnet eller
...
besläktad med laplacetransformen och Z-transformen.
%, dessa kan i det närmaste betraktas som specialfall av fouriertransformen.
...
%Social dimension - Bygga upp och bevara sociala institutioner och strukturer som är viktiga för mänskligt välbefinnande. T.ex så är det viktigt för universitet att upprätthålla kunskap och ideer som idag inte är så centrala men som kan vara viktigt i framtiden. Undervisningsorganisationer etc. 

Ett läromaterial som \emph{lyckas} få datastudenter att \emph{förstå} innehållet i kursen som annars skulle finna den svår kommer att leda till positiva påverkan i den sociala dimension. %Kan någon hjälpa mig formulera om detta?

Det kortsiktiga resultatet från läromaterialet är att andelen godkända studenter i kursen TSS höjs. Därför kan en säga att välbefinnandet förbättras för studenterna som annars inte skulle klara kursen eftersom studenten har mindre tentamen efter sig.

%---------------------OBS detta kanske är för dramatiskt....---------
Det långsiktiga målet för projektet DSLsofMath är dock att minska gapet mellan matematik och programmering. %Kanske formuleras om?
Detta kommer att öppna upp flera möjligheter för att tackla framtida utmaningar eftersom det blir i framtiden fler ingenjörer som inte är begränsade till enbart ett tankesätt.
Läromaterialet är då ett \emph{social support} som stödjer studenterna med att få en bättre förståelse inom båda domänen. Detta förbättrar inte bara till studenternas utan möjligen även individerna de framtida ingenjörerna kommer att påverka.

Alla dessa möjliga påverkan resulteras från en kompetent läromaterial som verkligen lyckas minska gapet mellan programmering och matematik. Detta är definitivt inte något som projektets resultat har lyckats uppnå vilket innebär att ytterliggare arbeten kommer att behövas.
...
% TODO: Ett intressant tillägg kunde vara att också reflektera över
% ert eget lärande: är utveckling av kursmaterial ett bra sätt att
% lära sig? I så fall kanske en del "future work" med fördel kunde
% utvecklas av andra studenter på programmet som ett led i deras
% lärprocess!
...
enskilda individer. Samtliga bilder i bilagan kommer från
